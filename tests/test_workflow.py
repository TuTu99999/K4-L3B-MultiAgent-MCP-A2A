...
class FakeGateway:
    ...
    async def call(self, tool_name: str, *, case_id: str, **arguments: str) -> dict[str, Any]:
        ...
        if tool_name == "get_order":
            ...
            data = {
                "events": [
                    {"event_type": "captured", "status": "confirmed", "amount_brl": 1.0}
                ]
            }
            domain = "payment_timeline"
        else:
            data = {"order_id": "a" * 32}
...
def test_seller_hypothesis_collects_shipment_without_redundant_seller_call(
    ...
    assert gateway.called_tools.count("get_shipment_summary") == 1
    assert "get_sellers" not in gateway.called_tools
    assert gateway.sequence == 6
